...
export default defineComponent({
    ...
    setup() {
        ...
        const autoValue = ref([{name:""}]);
        const selectedAutoValue = ref(null);
        const autoFilteredValue = ref([{name:""}]);
        ...
        const message = ref([{severity: "", content:"" }]);
        ...
        const onContextRightClick = (event: any) => {
            contextMenu.value.show(event);
        };
        const searchCountry = (event: any) => {
            setTimeout(() => {
                if (!event.query.trim().length && autoValue.value !== null) {
        ...
        const addMessage = (severity: string) => {
        ...
        const confirmWithPopup = (event: any) => {
            confirm.require({
